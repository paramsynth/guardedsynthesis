...
def p_section_data_signals_definitions(p):
    """signals : template_signal_name SEP
               | signals template_signal_name SEP """
    if len(p) == 4:
        p[0] = p[1] + [p[2]]
    else:
...
def p_template_signal_name(p):
    """ template_signal_name : SIGNAL_NAME
    """
    name_parts = p[1].split('_')
    assert(len(name_parts) > 1)
    p[0] = TemplateSignal('_'.join(name_parts[:-1]), int(name_parts[-1]))

def p_section_data_settings(p):
    """settings : setting
                | settings setting"""
    if len(p) == 2:
        p[0] = [p[1]]
...
def p_section_data_setting(p):
    """setting : SIGNAL_NAME ':' NUMBER"""
    p[0] = (p[1], p[3])
...
class QuantifiedSignalsCreatorVisitor(Visitor):
    ...
    def visit_signal(self, signal:Signal):
        ...
        base_name_tokens = [t for t in tokens if t not in self.binding_indices] # get all parts (regarding '_')
        indices_tokens = tuple(t for t in tokens if t not in base_name_tokens)
        assert('_'.join(base_name_tokens) + '_' + '_'.join(indices_tokens) == signal.name)
        template_token = int(base_name_tokens[-1])
        base_name_tokens = base_name_tokens[:-1]
        

        return QuantifiedTemplateSignal('_'.join(base_name_tokens), template_token, *indices_tokens)
